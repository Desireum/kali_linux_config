...
//                console.log('docGuid:' + docGuid + ' title:' + title + ' message' + chrome.i18n.getMessage('clipResult_success'));
                    ...
                    var errMessage = chrome.i18n.getMessage('clipResult_error');
                    if (error.status === 413) {
                        errMessage = chrome.i18n.getMessage('err_413');
                    }
        ...
        onGetUserInfo: function (data, error, port) {
            if (data && data.code === 200) {
                ...
                Wiz.Remote.setKbServer(data.result.kbServer);
                _data.saveUserInfo();
        ...
        onLogin: function (data, error, port, callback) {
            ...
            if (!data || data.code !== 200) {
                if (port) {
                    errMsg = (data && data.code) ? {code: data.code, returnMessage: data.returnMessage} : error;
                    try {
